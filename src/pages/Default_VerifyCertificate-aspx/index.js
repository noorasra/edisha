...
const MarriageDataList = () => {
  ...
  const { tid, cid, aid } = router.query; // Get query parameters from the URL
  ...
  const generatePDF = () => {
    ...
    html2canvas(input, { scale: 2 }).then((canvas) => {
      ...
      pdf.save("Default_VerifyCertificate.pdf"); // Name of the PDF file
  ...
  useEffect(() => {
    // Only fetch data if tid, cid, and aid are available
    ...
    fetchData();
  }, [tid, cid, aid]); // Run fetchData when tid, cid, or aid changes
  ...
  useEffect(() => {
    // Generate barcode when the component mounts or data loads
